...
import {
  Plus, Search, Filter, Eye, Edit, Trash2, Archive, MessageSquare, Upload, Edit2, Check, ExternalLink
} from 'lucide-react';
...
export const Inventory: React.FC = () => {
  ...
  // ===== Édition inline du lien de suivi =====
  ...
  const saveTracking = async (phoneId: string) => {
    try {
      const { error } = await supabase
        .from('phones')
        .update({ tracking_link: linkDraft })
        .eq('id', phoneId);

      if (error) throw error;

      setPhones((prev) =>
        prev.map((p) => (p.id === phoneId ? { ...p, tracking_link: linkDraft } : p))
      );
      setEditingTrackingId(null);
      showToast('Lien de suivi mis à jour', 'success');
    } catch {
        ...
        {filteredPhones.map((phone) => {
          ...
          return (
            <div key={phone.id} className="backdrop-blur-xl bg-white/5 border border-white/10 rounded-2xl p-4 hover:bg-white/10 transition-all duration-300 group">
              ...
              {/* Lien de suivi — visible uniquement en Arrivage */}
              {status === 'incoming' && (
                <div className="mb-3 px-3 py-2 bg-black/20 rounded-xl">
